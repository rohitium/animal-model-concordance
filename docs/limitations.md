...
**L91 — Label-based and trial-based pairs answer different questions, and cannot be pooled without saying so.** 169 of the 799 classified pairs take their human result from a US label indication. A label is strong evidence: it means the FDA reviewed adequate and well-controlled trials and accepted the indication. The problem is not its quality but its range. Of those 169 pairs, 164 are human-positive, 5 indeterminate, and **none is human-negative** — a label exists because a drug worked, so a label-based pair can only ever disagree in one direction (veterinary evidence negative against a positive human record). Concordance is 85% (94/110) in that subset and 76% (100/132) where the human side is a trial or meta-analysis, and the gap is a consequence of that structural asymmetry rather than of labels being weaker evidence. The two subsets should be reported separately, or the reader will take a number carrying 169 pairs that cannot produce a human failure as though it measured the same thing as one that can.

An earlier version of this entry claimed that scoring veterinary trials against labels inflated concordance and that such comparisons were not like-for-like. That was wrong, and the author said so: a label indication is a valid and strong human result. The correction is recorded rather than silently replaced.

**L92 — Where the human result is negative, the veterinary evidence usually said the opposite.** 42 classified pairs have a negative human result, and 23 of those have a determinate veterinary side. The veterinary evidence called it negative in 7 and positive in 16, so it anticipated the human failure in 7 of 23 (30%); 17 of the 23 are dog pairs and 6 are cat. This is the subset that tests whether animal evidence predicts *failure* rather than merely agreeing about success, and it is ten times larger than the 4 human-negative pairs inside the Q4 three-way-determinate set that the site's Q4 caveat describes. It points the same way as Q4 and more strongly: the corpus-wide agreement figure is carried by drugs that work in both species, and where the human answer turned out to be "no", the veterinary evidence mostly said "yes". Computed from the committed pair records; it is not yet a protocol-stage analysis, so it is recorded here rather than presented as a site headline.

**L93 — Most discontinuation reasons cannot be established, and they are held back rather than counted.** Candidacy for the shelved-asset route requires a verified non-clinical reason for a programme stopping (A18). Of the discontinued molecules reaching that gate, 213 have no reason legible in the source held, and they are excluded from candidates rather than presented with a guess. This is a deliberate trade: the route that is most attractive in principle — a molecule whose expensive safety package is already paid for — is the one this data can least often support, and it yields exactly one verified candidate. Treat the 213 as unexamined inventory, not as rejected inventory; each would need its trial record read before it could be ruled in or out. Relatedly, 5 of the 54 route-1 molecules (furosemide, tadalafil, losartan, triamcinolone, betamethasone) are already routine veterinary care as generics. They qualify only because no *company programme* in the supplied 328 targets them, which is not the same as no dog receiving them; they are flagged on the site, but the flag is hand-written and incomplete.

**L94 — The drug-pair corpus measures the published veterinary trial literature, not the veterinary formulary.** A pair exists where a published study tested a drug in companion animals with naturally occurring disease and that drug also exists in human medicine; the veterinary side comes from a systematic PubMed frame plus the NCI Comparative Oncology Trials Consortium list. Checked against a supplied list of 328 branded companion-animal programmes, 118 have no human counterpart by indication (parasiticides, vaccines, supplements, devices), 143 are represented somewhere in the pair records, and 67 are marketed companion-animal drugs absent from the corpus — among them Vetmedin (pimobendan), Varenzin (molidustat), Stelfonta (tigilanol tiglate), Zycortal, Pexion (imepitoin) and Slentrol. Their absence is not an error in the pair pipeline: it reflects that a marketed veterinary drug need not carry a retrievable published trial literature. But it means the pair count is a lower bound on the human-to-veterinary relationship, and that any claim about how *much* of veterinary practice descends from human medicine must not be read off this corpus.

**L95 — No approved-animal-drug registry is in the loop, so "no companion-animal programme" is a statement about four named sources.** Companion-animal presence is established from a curated mechanism map over a supplied list of 328 branded programmes, a curated human-indication to companion-condition map over the same list, a deterministic PubMed veterinary literature search, and this review's own pair corpus. It is **not** established against any register of approved veterinary medicines: the FDA Green Book publishes no machine-readable export, openFDA's animal endpoint returns adverse events rather than approved products, and the EMA veterinary dataset has moved behind a portal with no download. A molecule approved for dogs or cats outside the supplied list would therefore show as "not found". **Superseded in part (A30).** The section explaining what "no companion-animal program" means was removed from the program-selection page on 2026-09-17. The candidate browser still reports presence per molecule - "mechanism unoccupied" on 80 candidates, "no program, and no veterinary literature found" on 6 - with nothing on that page stating these are findings about the four sources named above rather than about the world. A reader of the table alone will read absence of a finding as absence of a program, which is the reading this entry previously said the site never permits. The checks are unchanged and are still recorded here; what was removed is the disclosure on the page. Adding a veterinary registry is the single highest-value fix available to this analysis.

**L96 — The curated presence map covers the mechanisms our candidates use, not the whole human pipeline.** The mechanism map was written against the condition areas where this review holds dog evidence, so it classifies 75 of the 78 candidates but only about a quarter of the 1,949 supplied human programmes; the rest fall into "mechanism not classified". That is adequate for the candidate list and inadequate for any statement of the form "X% of human drugs reach companion animals" — such a figure computed from this map would largely report the map's own coverage. No such figure is published, and the flow diagrams on the site are drawn only over stages where every programme is accounted for.

**L97 — Patent filings and abandoned programmes are searched by hand, not systematically, so an unclaimed mechanism may simply be an unsearched one.** Companion-animal presence was originally established from marketed products only, which misses two kinds of occupancy that matter for programme selection: intellectual property, and assets a company tried and dropped. Anti-IL-4Rα is the demonstration. No IL-4R product is marketed for dogs, so a marketed-products view returned "mechanism unoccupied" — while Merck Animal Health (Intervet) holds a granted patent on anti-canine IL-4Rα with 2015 priority plus further filings including a caninized anti-IL-4Rα and an atopic-dermatitis fusion protein, Vetoquinol filed WO2025114614A1 in December 2024, Kindred Biosciences filed veterinary anti-IL-4 receptor antibodies in 2019, and Elanco recorded a $53 million impairment in Q2 2024 terminating its own pet-health IL-4R asset for commercial reasons. Patents and halted programmes are now recorded per mechanism with their sources, but **only for the mechanisms where someone looked**: a mechanism carrying no patent entry has not been searched and must not be read as free of intellectual property. No automated sweep is possible at present — Google Patents blocks scripted and browser querying alike, PatentsView requires a registered API key, and EPO OPS requires OAuth credentials. Obtaining one of those keys would convert this from a hand-curated sample into a systematic check, and is the second highest-value fix available after a veterinary drug registry (L95).

**L98 — The unassigned-species results are mostly aggregate findings, not vague papers, and a quarter of them are our own parsing.** The evidence map previously said these results "name their animals only as 'animals' or 'rodents', so totals are lower bounds". Classifying all 259 shows that is wrong in both directions. **188 (73%)** are genuinely aggregate: meta-analyses and translatability reviews whose unit of analysis is animal models as a class ("the concordance between animal studies and clinical studies varied for 6 interventions"), with no species to recover and no species total being depressed. **41 (16%)** name two or more species and are rejected by `_one_animal()`, which returns a species only when exactly one is found — 21 of those name the species directly in `species_as_reported`, including one reading "dogs, rats, mice, rabbits and monkeys". Counting each toward every species it names would add mouse +29, rat +19, zebrafish +9, dog +8, pig +8. **16 (6%)** carry "humans" or "patients" in `species_as_reported`, the human side of the comparison stored in the animal field, across 6 studies. **14 (5%)** name no animal anywhere. The prose now describes this split instead of blaming the sources. **Superseded in part (A27):** this split was computed over 259 off-map results, before multi-species assignment (A25). The corpus now holds 244 off-map results and none of them names two or more species, so the 41 (16%) bucket no longer exists and the remaining percentages no longer sum against the same denominator. The site no longer publishes these figures; recomputing them requires re-deciding where an aggregate finding ends and a generic label begins, which has not been done. The counts are deliberately left unchanged: 5 of the 8 dog-naming results carry `model_type` of `induced` or `mixed-or-not-stated` and would land in laboratory-dog, so admitting them would shift the companion-versus-laboratory split — the comparison the review's conclusions rest on — on the strength of a species parsed from free text. Under A10 those model types are read from full text before being trusted, and that reading has not been done for these studies.
