...
**L91 — Pair concordance is inflated by scoring veterinary trials against US labels rather than against trials.** Of the 242 pairs with a determinate verdict, 110 take their human result from a US label indication (`human_top_level: us-approval`) rather than from a trial in the same indication. Concordance is 85% (94/110) on those pairs, against 76% (100/132) where the human side is a trial or meta-analysis. A label indication is effectively a guaranteed positive, so any pair scored against one is pre-loaded toward agreement, and the direction of the bias is upward. The asymmetry in the discordant pairs shows the same thing from the other side: of the 32 pairs where the veterinary evidence was negative and the human evidence positive, 16 rest on a label — bupivacaine's approval for local anaesthesia against a canine RCT that missed its pain endpoint, doxorubicin's label listing metastatic bone sarcoma against a canine trial where adding it worsened disease-free interval. Neither is a like-for-like comparison. The strata table on the site reports the published figure and is not restated here; this limitation says what that figure is partly made of.
